...
class RecommendationEngine:
    ...
    def __init__(self):
        ...
        selectedService = (
            Service.AzureOpenAI
            if config.get("USE_AZURE_OPENAI") == "True"
            else Service.OpenAI
        )
    ...
    async def get_recommendations(self, keyword_phrase, previous_links_str=None):
        ...
        if previous_links_str is not None:
            prompt = (
                prompt
                + f". EXCLUDE the following links from your recommendations: {previous_links_str}"
            )
        ...
        try:
            ...
            recommendation = [
                {"title": "Sorry, unable to recommendation at this time", "link": ""}
            ]
